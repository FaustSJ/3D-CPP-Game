...
private:
	...
	bool visited;		//for pathfinding
	GridNode* prevNode;	//for pathfinding
	int heuristic;		//the node's heuristic for pathfinding
	int cost;			//the node's pathCost for pathfinding
...
public:
	...
	void reset();	//it costs a ton of RAM, but I need it for pathfinding!
	void softReset();
	...
	bool notVisited();		//for pathfinding
	bool isClear();			// is the node walkable
	GridNode* getPrevNode();	//for pathfinding
	int getHeuristic();			//for pathfinding
	int getCost();				//for pathfinding
	bool isHeuristicSet();		//for pathfinding
	//setters
	void setID(int id);		// set the node id
	void setRow(int r);		// set the row coordinate
	void setColumn(int c);		// set the column coordinate
	void setPrevNode(GridNode* pn);			//for pathfinding
	void setClear();		// set the node as walkable
	void setOccupied();		// set the node as occupied
	void setVisited();		//for pathfinding
	void setHeuristic(int i);	//for pathfinding
	void setCost(int i);		//for pathfinding
...
public:
	...
	*/
	...
	Ogre::Vector3 getGridPosition(int r, int c); //returns the position on the grid
};
